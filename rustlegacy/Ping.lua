...
function PLUGIN:PingKick(player)
    ...
    if self.Config.Settings.PingKick == "true" then
        if ping >= self.Config.Settings.MaxPing then
            local message = ParseString(self.Config.Messages.PlayerKicked, { player = player.displayName, ping = ping })
    ...
    if args.Length == 1 then
        ...
        if targetPlayer then
            ...
            if HasPermission(steamId, "ping.bypass") then
                local message = ParseString(self.Config.Messages.PlayerExcluded, { player = targetPlayer.displayName })
                rust.SendChatMessage(player, message)
                return
            end
            local ping = self:PingKick(targetPlayer)
            local message = ParseString(self.Config.Messages.PlayerCheck, { player = targetPlayer.displayName, ping = ping })
            rust.SendChatMessage(player, message)
        end
    else
        local ping = Ping(player)
        local message = ParseString(self.Config.Messages.SelfCheck, { player = player.displayName, ping = ping })
        rust.SendChatMessage(player, message)
    end
end
