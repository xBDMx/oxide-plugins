...
function PLUGIN:cmdAuthLevel(player, cmd, args)
    ...
    local message = self.Config.Messages.AuthLevelSet:gsub("{level}", authLevel)
    local message = message:gsub("{player}", targetPlayer.displayName)
...
function PLUGIN:ccmdAuthLevel(args)
    ...
    if targetPlayer == nil then
        args:ReplyWith(self.Config.Messages.InvalidTarget)
        return
    end
...
function PLUGIN:LoadDefaultConfig()
    ...
    self.Config.Settings.HelpChatName = nil -- Removed in 0.1.3
    self:SaveConfig()
end
