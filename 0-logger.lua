...
function PLUGIN:Init()
    ...
    if plugins.Find("chathandler") then compatible = false end
end
        ...
        if self:PermissionsCheck(player) then
            ...
            if not excluded then
                ...
                if self.Config.Settings.Broadcast ~= "false" then
                    global.ConsoleSystem.Broadcast("chat.add \"" .. self.Config.Settings.ChatName .. "\" \"" .. player.displayName .. " ran console command: " .. console .. "\"")
                end
            elseif chat:sub(1, 1) == "/" then
                print("[" .. self.Title .. "] " .. player.displayName .. " ran chat command: " .. chat)
                if self.Config.Settings.Broadcast ~= "false" then
                    global.ConsoleSystem.Broadcast("chat.add \"" .. self.Config.Settings.ChatName .. "\" \"" .. player.displayName .. " ran chat command: " .. chat .. "\"")
                end
            end
        end
    end
...
function PLUGIN:OnPlayerChat(arg)
    ...
    local player = arg.connection.player
    local chat = arg:GetString(0, "text")
    if self.Config.Settings.ChatLogging ~= "false" and compatible ~= "false" then if chat:sub(1, 1) ~= "/" and chat ~= "" then print(player.displayName .. ": " .. chat) end end
end
...
function PLUGIN:LoadDefaultConfig()
    ...
    self.Config.Settings.Exclusions = self.Config.Settings.Exclusions or {
        "chat.say", "craft.add", "craft.cancel", "global.kill", "global.respawn", "global.respawn_sleepingbag", "global.status", "global.wakeup"
    }
    self:SaveConfig()
end
