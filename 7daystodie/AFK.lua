...
PLUGIN.Url = "http://oxidemod.org/plugins/1028/"
PLUGIN.ResourceId = 1028
...
function PLUGIN:CheckPosition(player)
    ...
    afkTimer[steamId] = timer.Repeat(timeLimit, 0, function()
        ...
        if start.x == current.x and start.y == current.y and start.z == current.z then
            ...
            local message = self.Config.Messages.PlayerKicked:gsub("{player}", player.EntityName)
            Print(self, message)
            if self.Config.Settings.Broadcast == "true" then sevendays.BroadcastChat(message) end
        end
...
function PLUGIN:OnPlayerConnected(player)
    ...
    if HasPermission(player.steamId, "afkkick.bypass") then return end

    self:CheckPosition(player)
end

function PLUGIN:OnPlayerDisconnected(client)
    local player = global.GameManager.Instance.World.GetEntity(client.entityId)
    if not player then return end
    local steamId = player.steamId
