...
{
    [Info("Ping", "Wulf / Luke Spragg", "1.0.1", ResourceId = 656)]
    ...
    class Ping : RustPlugin
    {
        ...
        */
        ...
        private Command cmdLib = Interface.Oxide.GetLibrary<Command>();
        ...
        void Loaded()
        {
            ...
            cmdLib.AddChatCommand(config.Settings.Command, this, ChatPing);
            cmdLib.AddConsoleCommand("global." + config.Settings.Command, this, ConsolePing);
        }

        int GetPing(Network.Connection connection) => Network.Net.sv.GetAveragePing(connection);
        ...
        object CanClientLogin(Network.Connection connection)
        {
            ...
            int ping = GetPing(connection);
            if (ping > config.Settings.MaxPing) return string.Format(config.Messages.Rejected, ping);
